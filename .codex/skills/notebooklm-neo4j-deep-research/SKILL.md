...
description: Iterative topic expansion and deep research across a NotebookLM notebook and a Neo4j graph built from the same source corpus. Use when Codex should answer or explore a topic by alternating between NotebookLM notebook queries and Neo4j concept-neighborhood queries, especially for concept discovery, follow-up question generation, gap finding, and multi-hop exploration from an initial research question.
---
...
- Read [neo4j-query-patterns.md](references/neo4j-query-patterns.md) before writing Cypher or adapting to the local schema.
...
```text
Use $notebooklm-neo4j-deep-research with NotebookLM notebook "quant-rnd" and the matching Neo4j graph.
...
Return the structured 8-section skill output.
```
