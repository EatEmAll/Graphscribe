#!/usr/bin/env python3
"""
Neo4j Graph Consolidation Quality Review - Fixed Version
"""
...
def run_consolidation_tests():
    """Run some basic consolidation tests."""
    ...
    try:
        ...
        with driver.session(database=database) as session:
            # Test potential for Tier 1 consolidation
            tier1_candidates = session.run("""
                MATCH (n:__Entity__)
                WHERE n.id IS NOT NULL
                WITH toLower(trim(n.id)) as normalized, count(*) as cnt
                WHERE cnt > 1
                RETURN count(*) as tier1_candidates
            """).single()["tier1_candidates"]

            print(f"Tier 1 Merge Candidates: {tier1_candidates}")

            # Test for entities without taxonomy
            no_taxonomy = session.run("""
                MATCH (n:__Entity__)
                WHERE NOT (n)-[:SUBCLASS_OF|INSTANCE_OF|TYPE_OF|IS_A]-()
                RETURN count(*) as no_taxonomy_count
            """).single()["no_taxonomy_count"]

            print(f"Entities without Taxonomy: {no_taxonomy}")

            # Test for consolidation metadata
            has_consolidation_meta = session.run("""
                MATCH (n)
                WHERE n.consolidated_on IS NOT NULL 
                   OR n.merged_from IS NOT NULL
                   OR n.tier_processed IS NOT NULL
                RETURN count(*) as consolidated_count
            """).single()["consolidated_count"]

            print(f"Nodes with Consolidation Metadata: {has_consolidation_meta}")
...
if __name__ == "__main__":
    report = analyze_consolidation_quality()
    if report:
