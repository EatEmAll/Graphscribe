...
## Hosted Neo4j and AuraDB

The corpus workflow supports AuraDB and other hosted Neo4j 5.23+ deployments. Use the provider's TLS URI and keep the password in an environment variable:
...
Use `--neo4j-password-env NAME` when different corpora use different credentials. The manifest stores `password_env` and connection metadata, never the password. REST, MCP, graph extraction, evaluation, and background sync resolve the same environment variable. Legacy corpus manifests containing a password are accepted with a warning and rewritten without it on the next save.

Before ingestion, the hosted preflight verifies database/write access, Neo4j version, the required uniqueness/range/full-text/vector schema, index readiness, and native query procedures. Corpus ingestion and retrieval do not require APOC. Use a fresh database or Aura instance for a v3 corpus when an existing legacy `Chunk.embedding` index has an incompatible name, dimension, or similarity function; the preflight fails rather than dropping existing schema.

## Verify a connection

`scripts/check_corpus_connection.py` is the read-only smoke test for an existing corpus. It resolves the connection from the manifest exactly as REST, MCP, graph extraction, evaluation, and background sync do, so a passing check means those paths can reach the same database. It never writes, so it is safe to run against a production corpus at any time.
...
The report covers connection identity (URI, database, user, resolved `password_env`, server address and version, connect latency), whether the manifest's `Corpus` node exists in the database, the manifest's configured vector and full-text indexes with their type, state, and population percent, and node/relationship counts. Add `--json` for a machine-readable report and `--relationship-limit N` to widen the relationship breakdown.

Exit codes are `0` when the corpus node is present and both retrieval indexes are `ONLINE`, `1` when the database is reachable but the corpus is missing or an index is not online, and `2` when the connection itself could not be established — a missing manifest, an unset password variable, an unresolvable host, or rejected credentials. A `2` with `Cannot resolve address` means the instance no longer exists at that hostname; a paused Aura instance still resolves and fails later, during the Bolt handshake.

Do not use `scripts/check_neo4j.py` for this. It reads the bare `NEO4J_URI`/`NEO4J_USERNAME`/`NEO4J_PASSWORD` variables and defaults to `bolt://localhost:7687`, so against a manifest-configured hosted corpus it either tests the wrong database or fails for the wrong reason.
...
### Move an existing v3 corpus to hosted Neo4j

For smaller corpora or a generic hosted provider, use portable mode. It copies nodes, relationships, vector properties, constraints, and indexes, then compares graph fingerprints and v3 corpus inventory before activating the new manifest target:

```powershell
$env:NEO4J_SOURCE_PASSWORD = "local-password"
$env:NEO4J_TARGET_PASSWORD = "hosted-password"

.\.venv\Scripts\python.exe scripts\migrate_neo4j.py `
...
Portable mode streams and fingerprints every embedding. For a large Aura corpus, prefer a whole-database upload from a Neo4j 5.26 LTS-or-newer managed container:

```powershell
.\.venv\Scripts\python.exe scripts\migrate_neo4j.py `
  --mode aura-upload `
  --source-container llm-graph-builder-neo4j-research-corpus `
  --source-uri bolt://127.0.0.1:7687 `
  --dump-dir C:\path\to\temporary-dumps `
  --target-uri neo4j+s://your-instance.databases.neo4j.io `
  --manifest-path data\corpora\research-corpus\manifest.json `
  --overwrite-target `
...
Both modes require an empty destination unless overwrite is explicitly confirmed. Post-migration verification checks corpus/document/revision/chunk counts, active embeddings, required schema, index state, and a vector-query smoke test. A NotebookLM-derived legacy graph is not converted into the v3 chunk/revision model by this command; build that corpus from original sources using the blue-green procedure above.
...
- Several corpora may share a compatible database, but native Neo4j 5.x vector/full-text ranking is global. Retrieval widens queries to preserve corpus-scoped recall; use a dedicated database or Aura instance per corpus at large scale to avoid that extra work.
