...
from src.adaptive_retry import (
    build_adaptive_retry_plan,
    describe_adaptive_retry_attempt,
    is_output_limit_error,
    run_chunk_batch_with_output_limit_fallback,
    run_with_adaptive_output_limit_fallback,
)
...
if GCS_FILE_CACHE:
    BUCKET_UPLOAD_FILE = get_value_from_env(
        'BUCKET_UPLOAD_FILE', default_value=None, data_type=str)
    BUCKET_FAILED_FILE = get_value_from_env(
        'BUCKET_FAILED_FILE', default_value=None, data_type=str)
    PROJECT_ID = get_value_from_env(
        'PROJECT_ID', default_value=None, data_type=str)

ADAPTIVE_RETRY_DELETE_DOCUMENT_STATE_QUERY = """
MATCH (d:Document {fileName: $filename})
OPTIONAL MATCH (d)<-[:PART_OF]-(c:Chunk)
OPTIONAL MATCH (c)-[:HAS_ENTITY]->(e)
WITH d, collect(DISTINCT c) AS chunks, collect(DISTINCT e) AS candidate_entities
WITH d, chunks,
     [entity IN candidate_entities
      WHERE entity IS NOT NULL
        AND NOT EXISTS {
            MATCH (entity)<-[:HAS_ENTITY]-(:Chunk)-[:PART_OF]->(other:Document)
            WHERE other.fileName <> $filename
        }] AS entities
FOREACH (chunk IN chunks | DETACH DELETE chunk)
FOREACH (entity IN entities | DETACH DELETE entity)
REMOVE d.errorMessage, d.retry_condition
"""

def cleanup_document_for_adaptive_retry(credentials, file_name: str) -> None:
    ...
    try:
        ...
        obj_source_node.status = "Ready to Reprocess"
...
def create_source_node_graph_url_s3(graph, params):
    ...
    for file_info in files_info:
        ...
        obj_source_node = sourceNode()
        obj_source_node.file_name = file_name.split(
            '/')[-1].strip() if isinstance(file_name.split('/')[-1], str) else file_name.split('/')[-1]
...
def create_source_node_graph_url_gcs(graph, params, credentials):
    ...
    lst_file_metadata = get_gcs_bucket_files_info(
        params.gcs_project_id, params.gcs_bucket_name, params.gcs_bucket_folder, credentials)
    for file_metadata in lst_file_metadata:
        obj_source_node = sourceNode()
        obj_source_node.file_name = file_metadata['fileName'].strip() if isinstance(
            file_metadata['fileName'], str) else file_metadata['fileName']
...
def create_source_node_graph_web_url(graph, params):
    ...
    try:
        ...
        if title:
            ...
            if existing_url != params.source_url:
                title = str(title) + "-" + \
                    str(last_url_segment(params.source_url)).strip()
        else:
    ...
    graphDb_data_Access = graphDBdataAccess(graph)
    graphDb_data_Access.create_source_node(obj_source_node)
    lst_file_name.append({'fileName': obj_source_node.file_name,
                         'fileSize': obj_source_node.file_size, 'url': obj_source_node.url, 'status': 'Success'})
    success_count += 1
    return lst_file_name, success_count, failed_count
...
def create_source_node_graph_url_youtube(graph, params):
    ...
    Returns:
        tuple: (list of file info dicts, success_count, failed_count)
    """
    youtube_url, language = check_url_source(
        source_type=params.source_type, yt_url=params.source_url)
    success_count = 0
    failed_count = 0
    lst_file_name = []
    obj_source_node = sourceNode()
...
def create_source_node_graph_url_wikipedia(graph, params):
    ...
    wiki_query_id, language = check_url_source(
        source_type=params.source_type, wiki_query=params.wiki_query)
    logging.info(
        f"Creating source node for {wiki_query_id.strip()}, {language}")
    pages = WikipediaLoader(query=wiki_query_id.strip(
    ), lang=language, load_max_docs=1, load_all_available_meta=True).load()
    if pages == None or len(pages) == 0:
        failed_count += 1
        ...
        lst_file_name.append({'fileName': obj_source_node.file_name, 'fileSize': obj_source_node.file_size,
                             'url': obj_source_node.url, 'language': obj_source_node.language, 'status': 'Success'})
    return lst_file_name, success_count, failed_count
...
async def extract_graph_from_file_local_file(credentials, params, merged_file_path):
    ...
    logging.info(
        f'Process file name :{params.file_name} from local file system')
    if params.retry_condition in ["", None] or params.retry_condition not in [DELETE_ENTITIES_AND_START_FROM_BEGINNING, START_FROM_LAST_PROCESSED_POSITION]:
        if GCS_FILE_CACHE:
            folder_name = create_gcs_bucket_folder_name_hashed(
                credentials.uri, params.file_name)
            file_name, pages = get_documents_from_gcs(
                PROJECT_ID, BUCKET_UPLOAD_FILE, folder_name, params.file_name)
        else:
            file_name, pages, file_extension = get_documents_from_file_by_path(
                merged_file_path, params.file_name)
        if pages == None or len(pages) == 0:
...
async def extract_graph_from_file_s3(credentials, params):
    ...
    if params.retry_condition in ["", None] or params.retry_condition not in [DELETE_ENTITIES_AND_START_FROM_BEGINNING, START_FROM_LAST_PROCESSED_POSITION]:
        if params.aws_access_key_id is None or params.aws_secret_access_key is None:
            raise LLMGraphBuilderException(
                'Please provide AWS access and secret keys')
        else:
            logging.info("Insert in S3 Block")
            file_name, pages = get_documents_from_s3(
                params.source_url, params.aws_access_key_id, params.aws_secret_access_key)
        if pages == None or len(pages) == 0:
            raise LLMGraphBuilderException(
                f'File content is not available for file : {file_name}')
...
async def extract_graph_from_web_page(credentials, params):
    ...
    if params.retry_condition in ["", None] or params.retry_condition not in [DELETE_ENTITIES_AND_START_FROM_BEGINNING, START_FROM_LAST_PROCESSED_POSITION]:
        ...
        if pages == None or len(pages) == 0:
            raise LLMGraphBuilderException(
                f'Content is not available for given URL : {params.source_url}')
        return await processing_source(credentials, params, pages)
    else:
        return await processing_source(credentials, params, [])
...
async def extract_graph_from_file_Wikipedia(credentials, params):
    ...
    if params.retry_condition in ["", None] or params.retry_condition not in [DELETE_ENTITIES_AND_START_FROM_BEGINNING, START_FROM_LAST_PROCESSED_POSITION]:
        file_name, pages = get_documents_from_wikipedia(
            params.wiki_query, params.language)
        if pages == None or len(pages) == 0:
            raise LLMGraphBuilderException(
...
async def extract_graph_from_file_gcs(credentials, params):
    ...
    if params.retry_condition in ["", None] or params.retry_condition not in [DELETE_ENTITIES_AND_START_FROM_BEGINNING, START_FROM_LAST_PROCESSED_POSITION]:
        file_name, pages = get_documents_from_gcs(
            params.gcs_project_id, params.gcs_bucket_name, params.gcs_bucket_folder, params.gcs_blob_filename, params.access_token)
        if pages == None or len(pages) == 0:
            raise LLMGraphBuilderException(
                f'File content is not available for file : {file_name}')
        return await processing_source(credentials, params, pages)
...
async def _processing_source_once(credentials, params, pages, merged_file_path=None, is_uploaded_from_local=None, attempt=None):
    """
    ...
    graphDb_data_Access = graphDBdataAccess(graph)
    # Resolve embedding params: fall back to env vars when not provided by client
    effective_embedding_provider = params.embedding_provider or get_value_from_env(
        "EMBEDDING_PROVIDER", "sentence-transformer")
    effective_embedding_model = params.embedding_model or get_value_from_env(
        "EMBEDDING_MODEL", "all-MiniLM-L6-v2")
    logging.info(
        f"Embedding: provider={effective_embedding_provider}, model={effective_embedding_model}")
    create_chunk_vector_index(
        graph, effective_embedding_provider, effective_embedding_model)
    attempt = attempt or build_adaptive_retry_plan(params)[0]
    active_retry_condition = params.retry_condition
    if attempt.get("rebuild_chunks"):
        active_retry_condition = None
    logging.info(
        "Extraction attempt for %s: %s",
        params.file_name,
    ...
    total_chunks, chunkId_chunkDoc_list = get_chunkId_chunkDoc_list(
        ...
        params.file_name,
        pages,
        attempt["token_chunk_size"],
        attempt["chunk_overlap"],
        active_retry_condition,
        credentials.email,
    )
    end_get_chunkId_chunkDoc_list = time.time()
    elapsed_get_chunkId_chunkDoc_list = end_get_chunkId_chunkDoc_list - \
        start_get_chunkId_chunkDoc_list
    logging.info(
    ...
    if len(result) > 0:
        if result[0]['Status'] != 'Processing':
            obj_source_node = sourceNode()
            status = "Processing"
            obj_source_node.file_name = params.file_name.strip() if isinstance(
                params.file_name, str) else params.file_name
            obj_source_node.status = status
            obj_source_node.total_chunks = total_chunks
            obj_source_node.model = params.model
            obj_source_node.embedding_model = params.embedding_model
            if active_retry_condition == START_FROM_LAST_PROCESSED_POSITION:
            ...
            if get_value_from_env("TRACK_USER_USAGE", "false", "bool"):
                track_token_usage(credentials.email, credentials.uri,
                                  0, params.model, operation_type="precheck")
            ...
            update_graph_chunk_processed = get_value_from_env(
                "UPDATE_GRAPH_CHUNKS_PROCESSED", 20, "int")
            # selected_chunks = []
            ...
            for i in range(0, len(chunkId_chunkDoc_list), update_graph_chunk_processed):
                ...
                logging.info(
                    f"Value of is_cancelled : {result[0]['is_cancelled']}")
                if bool(is_cancelled_status) == True:
                    ...
                    async def process_selected_chunks(active_attempt: dict):
                        return await processing_chunks(
                            selected_chunks,
                            graph,
                            credentials,
                            params.file_name,
                            params.model,
                            params.allowedNodes,
                            params.allowedRelationship,
                            active_attempt["chunks_to_combine"],
                            node_count,
                            rel_count,
                            params.additional_instructions,
                            effective_embedding_provider,
                            effective_embedding_model,
                            low_verbosity=active_attempt["low_verbosity"],
                        )

                    node_count, rel_count, latency_processed_chunk, token_usage = await run_chunk_batch_with_output_limit_fallback(
                        params,
                        attempt,
                        process_selected_chunks,
                        file_name=params.file_name,
                        chunk_start_index=i,
                        chunk_end_index=select_chunks_upto,
                    )
                    logging.info(
                        "Token used in processing chunks: %s", token_usage)
                    tokens_per_file += token_usage
                    logging.info("Total token used per file: %s",
                                 tokens_per_file)
                    start_save_token = time.time()
                    try:
                        if get_value_from_env("TRACK_USER_USAGE", "false", "bool"):
                            track_token_usage(
                                credentials.email, credentials.uri, token_usage, params.model, operation_type="extraction")
                            logging.info(
                                "Token usage for extraction: %s for user: %s", token_usage, credentials.email)
                    ...
                    logging.info(
                        f'Time taken to save token count: {elapsed_save_token:.2f} seconds')

                    processing_chunks_end_time = time.time()
                    processing_chunks_elapsed_end_time = processing_chunks_end_time - \
                        processing_chunks_start_time
                    logging.info(
                        f"Time taken {update_graph_chunk_processed} chunks processed upto {select_chunks_upto} completed in {processing_chunks_elapsed_end_time:.2f} seconds for file name {params.file_name}")
                    ...
                    obj_source_node.processed_chunk = select_chunks_upto+select_chunks_with_retry
                    obj_source_node.token_usage = tokens_per_file
                    if active_retry_condition == START_FROM_BEGINNING:
                        result = execute_graph_query(graph, QUERY_TO_GET_NODES_AND_RELATIONS_OF_A_DOCUMENT, params={
            ...
            logging.info(
                f'file:{params.file_name} extraction has been completed')
            ...
            if is_uploaded_from_local and bool(is_cancelled_status) == False:
                if GCS_FILE_CACHE:
                    ...
                    delete_file_from_gcs(
                        BUCKET_UPLOAD_FILE, folder_name, params.file_name)
                else:
                    delete_uploaded_local_file(
                        merged_file_path, params.file_name)
            processing_source_func = time.time() - processing_source_start_time
            logging.info(
                f"Time taken to processing source function completed in {processing_source_func:.2f} seconds for file name {params.file_name}")
            ...
            response["total_processing_time"] = round(
                processed_time.total_seconds(), 2)
...
async def processing_source(credentials, params, pages, merged_file_path=None, is_uploaded_from_local=None):
    attempt_plan = build_adaptive_retry_plan(params)
    first_error_message: str | None = None

    try:
        return await _processing_source_once(
            credentials,
            params,
            pages,
            merged_file_path=merged_file_path,
            is_uploaded_from_local=is_uploaded_from_local,
            attempt=attempt_plan[0],
        )
    except LLMGraphBuilderException as exc:
        if not is_output_limit_error(str(exc)) or len(attempt_plan) <= 2:
            raise
        ...
        logging.warning(
            ...
            params.file_name,
            first_error_message,
            describe_adaptive_retry_attempt(attempt_plan[2]),
        )
        cleanup_document_for_adaptive_retry(credentials, params.file_name)

    async def attempt_runner(attempt: dict):
        return await _processing_source_once(
            credentials,
            params,
            pages,
    ...
    return await run_with_adaptive_output_limit_fallback(
        params,
        attempt_runner,
        cleanup_runner,
        start_index=2,
        initial_error_message=first_error_message,
    )
...
async def processing_chunks(
    ...
    create_chunk_embeddings(graph, chunkId_chunkDoc_list,
                            file_name, embedding_provider, embedding_model)
    ...
    merge_relationship_between_chunk_and_entites(
        graph, chunks_and_graphDocuments_list)
...
def get_chunkId_chunkDoc_list(graph, file_name, pages, token_chunk_size, chunk_overlap, retry_condition, email):
    ...
    if retry_condition in ["", None] or retry_condition not in [DELETE_ENTITIES_AND_START_FROM_BEGINNING, START_FROM_LAST_PROCESSED_POSITION]:
        ...
        for i in range(0, len(pages)):
            ...
            for j in bad_chars:
                if j == '\n':
                    ...
                    text = text.replace(j, '')
            pages[i] = Document(page_content=str(
                text), metadata=pages[i].metadata)
        create_chunks_obj = CreateChunksofDocument(pages, graph)
        chunks = create_chunks_obj.split_file_into_chunks(
            token_chunk_size, chunk_overlap, email)
        chunkId_chunkDoc_list = create_relation_between_chunks(
            graph, file_name, chunks)
        return len(chunks), chunkId_chunkDoc_list

    else:
        chunkId_chunkDoc_list = []
        chunks = execute_graph_query(graph, QUERY_TO_GET_CHUNKS, params={
                                     "filename": file_name})
        ...
        if chunks[0]['text'] is None or chunks[0]['text'] == "" or not chunks:
            ...
            for chunk in chunks:
                chunk_doc = Document(page_content=chunk['text'], metadata={
                                     'id': chunk['id'], 'position': chunk['position']})
            ...
            if retry_condition == START_FROM_LAST_PROCESSED_POSITION:
                ...
                starting_chunk = execute_graph_query(
                    graph, QUERY_TO_GET_LAST_PROCESSED_CHUNK_POSITION, params={"filename": file_name})
                ...
                if starting_chunk and starting_chunk[0]["position"] < len(chunkId_chunkDoc_list):
                    ...
                    starting_chunk = execute_graph_query(
                        graph, QUERY_TO_GET_LAST_PROCESSED_CHUNK_WITHOUT_ENTITY, params={"filename": file_name})
                    return len(chunks), chunkId_chunkDoc_list[starting_chunk[0]["position"] - 1:]
...
def get_source_list_from_graph(credentials):
    ...
    graph = Neo4jGraph(url=credentials.uri, database=credentials.database,
...
def merge_chunks_local(file_name, total_chunks, chunk_dir, merged_dir):
    ...
    with open(merged_file_path, "wb") as write_stream:
        for i in range(1, total_chunks + 1):
            ...
            logging.info(
                f'Chunk File Path While Merging Parts:{chunk_file_path}')
...
def upload_file(graph, model, chunk, chunk_number: int, total_chunks: int, file_name, uri, chunk_dir, merged_dir):
    ...
    if GCS_FILE_CACHE:
        ...
        upload_file_to_gcs(chunk, chunk_number, safe_file_name,
                           BUCKET_UPLOAD_FILE, folder_name)
    else:
        if not os.path.exists(chunk_dir):
            os.mkdir(chunk_dir)
        chunk_file_path = os.path.join(
            chunk_dir, f"{safe_file_name}_part_{chunk_number}")
    ...
    if int(chunk_number) == int(total_chunks):
        ...
        if GCS_FILE_CACHE:
            file_size = merge_file_gcs(
                BUCKET_UPLOAD_FILE, safe_file_name, folder_name, int(total_chunks))
        else:
            file_size = merge_chunks_local(
                safe_file_name, int(total_chunks), chunk_dir, merged_dir)
        logging.info("File merged successfully")
        file_extension = safe_file_name.split('.')[-1]
        obj_source_node = sourceNode()
        obj_source_node.file_name = safe_file_name.strip() if isinstance(
            safe_file_name, str) else safe_file_name
...
def get_labels_and_relationtypes(credentials):
    ...
    """
    excluded_labels = {'Document', 'Chunk', '_Bloom_Perspective_',
                       '__Community__', '__Entity__', 'Session', 'Message'}
...
def manually_cancelled_job(graph, filenames, source_types, merged_dir, uri):
    ...
    for (file_name, source_type) in zip(filename_list, source_types_list):
        ...
        ) if isinstance(file_name, str) else file_name
...
def set_status_retry(graph, file_name, retry_condition):
    ...
    if retry_condition == DELETE_ENTITIES_AND_START_FROM_BEGINNING:
        execute_graph_query(graph, QUERY_TO_DELETE_EXISTING_ENTITIES, params={
                            "filename": file_name})
...
def failed_file_process(uri, file_name, merged_file_path):
    ...
    if GCS_FILE_CACHE:
        ...
        copy_failed_file(BUCKET_UPLOAD_FILE,
                         BUCKET_FAILED_FILE, folder_name, file_name)
        time.sleep(5)
