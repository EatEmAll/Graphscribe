...
def _get_sentence_transformer_embedding(model_name: str, model_path: str = "./local_model"):
    ...
    """
    # Fix HuggingFace identifier (requires namespace)
    hf_model_name = "sentence-transformers/" + \
        model_name if "/" not in model_name else model_name

    if hf_model_name not in _embedding_locks:
        _embedding_locks[hf_model_name] = Lock()
    if hf_model_name in _embedding_instances:
        return _embedding_instances[hf_model_name]
    with _embedding_locks[hf_model_name]:
        if hf_model_name in _embedding_instances:
            return _embedding_instances[hf_model_name]
        _ensure_sentence_transformer_model_downloaded(
            hf_model_name, model_path)
        _embedding_instances[hf_model_name] = HuggingFaceEmbeddings(
...
def create_graph_database_connection(credentials):
    enable_user_agent = get_value_from_env(
        "ENABLE_USER_AGENT", "False", "bool")
    if enable_user_agent:
        graph = Neo4jGraph(url=credentials.uri, database=credentials.database, username=credentials.userName, password=credentials.password,
                           refresh_schema=False, sanitize=True, driver_config={'user_agent': get_value_from_env("USER_AGENT", "LLM-Graph-Builder")})
    else:
        graph = Neo4jGraph(url=credentials.uri, database=credentials.database, username=credentials.userName,
                           password=credentials.password, refresh_schema=False, sanitize=True)
    return graph
    ...
    while retries < max_retries:
        try:
            graph.add_graph_documents(
                graph_document_list, baseEntityLabel=True)
            return
...
def handle_backticks_nodes_relationship_id_type(graph_document_list: List[GraphDocument]):
    for graph_document in graph_document_list:
        ...
        for rel in graph_document.relationships:
            ...
            src_type = str(getattr(src, "type", "")
                           ).strip() if src is not None else ""
            src_id = str(getattr(src, "id", "")
                         ).strip() if src is not None else ""
            tgt_type = str(getattr(tgt, "type", "")
                           ).strip() if tgt is not None else ""
            tgt_id = str(getattr(tgt, "id", "")
                         ).strip() if tgt is not None else ""
...
def last_url_segment(url):
    ...
    last_url_segment = path.split(
        "/")[-1] if path else parsed_url.netloc.split(".")[0]
...
def convert_type(value: str, data_type: type):
    ...
    try:
        if data_type in (int, "int"):
            ...
            raise TypeError(
                f"Unsupported data type for conversion: {data_type}")
    except Exception as e:
...
class UniversalTokenUsageHandler(BaseCallbackHandler):
    ...
    def on_llm_end(self, response, **kwargs):
        usage = response.llm_output.get(
            "token_usage") if response.llm_output else None

        if usage:
            self.total_prompt_tokens += usage.get(
                "prompt_tokens") or usage.get("input_tokens") or 0
            self.total_completion_tokens += usage.get(
                "completion_tokens") or usage.get("output_tokens") or 0
            return

        for generations in response.generations:
            for generation in generations:
                if hasattr(generation, 'message'):
                    metadata = getattr(generation.message,
                                       'usage_metadata', {})
                    if metadata:
                        self.total_prompt_tokens += metadata.get(
                            "input_tokens", 0)
                        self.total_completion_tokens += metadata.get(
                            "output_tokens", 0)
...
def track_token_usage(
    ...
    try:
        ...
        logging.info(
            f"Tracking token usage for email: {normalized_email}, uri: {normalized_db_url}, usage: {usage}, operation_type: {operation_type}")
        is_neo4j_user = bool(
        ...
        if not normalized_email and not normalized_db_url:
            raise ValueError(
                "Either email or db_url must be provided for token tracking.")
        ...
        token_database = get_value_from_env(
            "TOKEN_TRACKER_DB_DATABASE", "neo4j")
        credentials = Neo4jCredentials(
            uri=token_uri, userName=token_user, password=token_password, database=token_database)
        if not all([token_uri, token_user, token_password]):
            raise EnvironmentError("Neo4j credentials are not set properly.")

        graph = create_graph_database_connection(credentials)
        daily_tokens_limit = get_value_from_env(
            "DAILY_TOKENS_LIMIT", "250000", "int")
        monthly_tokens_limit = get_value_from_env(
            "MONTHLY_TOKENS_LIMIT", "1000000", "int")
        auth_required = get_value_from_env(
            "AUTHENTICATION_REQUIRED", False, bool)
        ...
        if auth_required:
            if not normalized_email:
                raise ValueError(
                    "Email is required for token tracking when authentication is required.")
            merge_query = """
            ...
            if not normalized_db_url:
                raise ValueError(
                    "db_url is required for token tracking when authentication is not required.")
        ...
        if result and "latestUsage" in result[0]:
            ...
            logging.info("Token usage for user before limit check: daily_used=%s monthly_used=%s",
                         daily_tokens_used, monthly_tokens_used)
            logging.info("Token limits for user: daily_limit=%s monthly_limit=%s",
        ...
        logging.error(
            "No matching User found or failed to fetch updated token usage. Result: %s", result)
        raise RuntimeError(
            "User not found or failed to fetch updated token usage.")
...
def get_remaining_token_limits(email: str, uri: str) -> dict:
    ...
    try:
        ...
        normalized_db_url = (uri or "").strip() or None
        logging.info(
            f"Fetching remaining token limits for email: {normalized_email}, uri: {normalized_db_url}")
        if not normalized_email and not normalized_db_url:
            raise ValueError(
                "Either email or db_url must be provided for token tracking.")
        ...
        if auth_required:
            ...
            if not normalized_email:
                raise ValueError(
                    "Email is required for token lookup when authentication is required.")
            result = graph.query(
            ...
            if not normalized_db_url:
                raise ValueError(
                    "db_url is required for token lookup when authentication is not required.")
            result = graph.query(
                "MATCH (u:User {db_url: $db_url}) RETURN u", {
                    "db_url": normalized_db_url}
            )
            logging.info(f"Query result for user lookup: {result}")
            if result and result[0].get("u"):
        ...
        if user_node:
            daily_tokens_limit = user_node.get(
                "daily_tokens_limit", daily_tokens_limit)
            monthly_tokens_limit = user_node.get(
                "monthly_tokens_limit", monthly_tokens_limit)
...
def get_user_embedding_model(email: str, uri: str) -> dict:
    ...
    try:
        ...
        track_user_usage = get_value_from_env(
            "TRACK_USER_USAGE", "false", bool)
        if not track_user_usage:
            ...
            _, embedding_dimension = load_embedding_model(
            ...
            if not all([token_uri, token_user, token_password]):
                allow_embedding_change = False
                raise EnvironmentError(
                    "Neo4j credentials are not set properly.")
        ...
        if not normalized_email and not normalized_db_url:
            raise ValueError(
                "Either email or db_url must be provided for fetching embedding model.")
        ...
        logging.error(
            f"Error in get_user_embedding_model for email {email}: {e}")
        return "sentence-transformer", "all-MiniLM-L6-v2", 384, allow_embedding_change
...
def change_user_embedding_model(email: str, uri: str, new_embedding_provider: str, new_embedding_model: str) -> dict:
    ...
    try:
        ...
        if not normalized_email and not normalized_db_url:
            raise ValueError(
                "Either email or db_url must be provided for changing embedding model.")
        ...
        tracker_credentials = Neo4jCredentials(
            uri=token_uri, userName=token_user, password=token_password, database=token_database)
        ...
        old_dimension = user_node.get(
            "embedding_dimension") if user_node else 384
        _, new_dimension = load_embedding_model(
            new_embedding_provider, new_embedding_model)
        if auth_required:
            update_query = """
            MATCH (u:User {email: $email})
            ...
            RETURN u
            """
            tracker_graph.query(update_query, {"email": normalized_email, "embedding_provider": new_embedding_provider,
                                "embedding_model": new_embedding_model, "embedding_dimension": new_dimension})
        else:
            update_query = """
            MATCH (u:User {db_url: $db_url})
            SET u.embedding_provider = $embedding_provider, u.embedding_model = $embedding_model, u.embedding_dimension = $embedding_dimension, u.updatedAt = datetime()
            RETURN u
            """
            tracker_graph.query(update_query, {"db_url": normalized_db_url, "embedding_provider": new_embedding_provider,
                                "embedding_model": new_embedding_model, "embedding_dimension": new_dimension})
        ...
        logging.error(
            f"Error in change_user_embedding_model for email {email}: {e}")
